...
class QuestionRequest(BaseModel):
    ...
    num_questions: int
    question_type: str = Field(..., pattern="^(MCQ|INT)$", description="Type of question")
...
def generate_questions(request: QuestionRequest):
    filtered_questions = [
        ...
        if q["difficulty"] == request.difficulty
        and q["type"] == request.question_type
        and (not request.pyqs or (q["pyqs"] and (request.year is None or q["year"] == request.year)))
        and (request.topic is None or q["topic"] == request.topic)
        and (request.subtopic is None or q["subtopic"] == request.subtopic)
    ]

    selected_questions = random.sample(filtered_questions, min(request.num_questions, len(filtered_questions)))
